...
async def get_user_input(
    ...
    """统一处理文本/语音输入，返回用户问题文本"""
    if input_mode == "text":
        return text_input.strip()
    elif input_mode == "voice" and audio_input is not None:
        return await transcribe_audio(audio_input)
    return ""


async def transcribe_audio(audio_input: Tuple[int, np.ndarray]) -> str:
    """语音转文本"""
    ...
    # 音频采样率兼容性
    assert sample_rate in [8000, 16000, 44100], "不支持的采样率"
...
async def process_input(
    ...
    try:
        ...
        # 获取用户输入
        user_input = await get_user_input(text_input, audio_input, input_mode)
        if not user_input:
            yield "输入内容为空"
            return
        ...
        # 流式处理
        full_response = ""
        async for text_chunk in query_rag_model_streaming(user_input):
            full_response += text_chunk
            yield full_response  # 更新前端显示

            # 异步播放当前文本块（不等待完成）
            asyncio.create_task(
                text_to_speech(text_chunk),
                name=f"tts-{hash(text_chunk)}"
            )
    except Exception as e:
       error_msg = f"处理错误: {str(e)}"
       print(error_msg)
       asyncio.create_task(
           text_to_speech("系统出错了，请稍后再试"))
       yield error_msg
...
def play_audio_chunk(audio_data: bytes):
    """实时播放音频二进制数据"""
    with tempfile.NamedTemporaryFile(suffix='.mp3', delete=False) as tmp:
        ...
        play(audio)
    os.unlink(tmp.name)  # 播放后立即删除临时文件
...
async def text_to_speech(text: str):
    try:
        ...
        async for chunk in communicate.stream():
            if chunk["type"] == "audio":
                play_audio_chunk(chunk["data"])  # 实时播放音频片段
    finally:
...
async def query_rag_model_streaming(prompt: str) -> AsyncGenerator[str, None]:
    try:
        # 获取RAG系统的流式响应
        async for chunk in RAG_SYSTEM.query_streaming(prompt):
            yield chunk
    except Exception as e:
        yield f"查询失败: {str(e)}"
